...
/*
 * Keep the reference count of each AnyMemoCentralDBOpenHelper.
 */
public class AnyMemoCentralDBOpenHelperManager {
    ...
    private static volatile Map<String, WeakReference<AnyMemoCentralDBOpenHelper>> helpers = Collections.synchronizedMap(new TreeMap<String, WeakReference<AnyMemoCentralDBOpenHelper>>(filenameComparator));
     ...
     */
    public static AnyMemoCentralDBOpenHelper getHelper(String dbpath) {
    ...
    public static AnyMemoCentralDBOpenHelper getHelper(Context context, String dbpath) {
        bigLock.lock();
        try {
            assert dbpath != null : "dbpath should not be null";
            if (!helpers.containsKey(dbpath)) {
                Log.i(TAG, "Call get AnyMemoCentralDBOpenHelper for first time for db: " + dbpath);
                AnyMemoCentralDBOpenHelper helper = new AnyMemoCentralDBOpenHelper(context, dbpath);
                helpers.put(dbpath, new WeakReference<AnyMemoCentralDBOpenHelper>(helper));
                refCounts.put(dbpath, 1);
                return helpers.get(dbpath).get();
            } else {
                Log.i(TAG, "Call get AnyMemoCentralDBOpenHelper for " + dbpath + " again, return existing helper.");
    ...
    public static void releaseHelper(AnyMemoCentralDBOpenHelper helper) {
        bigLock.lock();
        try {
            ...
            Log.i(TAG, "Release AnyMemoCentralDBOpenHelper: " + dbpath + " Ref count: " + refCounts.get(dbpath));
    ...
    public static void forceRelease(String dbpath) {
        ...
        try {
            ...
            AnyMemoCentralDBOpenHelper helper = helpers.get(dbpath).get();
